...
namespace Housing01
{
    public partial class RentlForm : Form
    {
        public RentlForm()
        {
            InitializeComponent();
        ...
        private void btnQuery_Click(object sender, EventArgs e)
        {
            ...
            {
                sqlStr = string.Format("select Rental.RentalID,Rental.HouseID,Rental.TenantID,TenantInfo.RealName,TenantInfo.Phone,Rental.RentStartDate,Rental.RentEndDate,Rental.MonthlyRent,Rental.DepositPaid,Rental.IsReturnDeposit,Rental.Remark from Rental,TenantInfo where Rental.TenantID=TenantInfo.TenantID and TenantInfo.RealName like '%{0}%'"
                    , this.tbValue.Text);
            }
        ...
        {
            this.tbStartDate.Text = this.dateTimePicker1.Text;
        }

        private void dateTimePicker2_ValueChanged(object sender, EventArgs e)
        {
            this.tbEndDate.Text = this.dateTimePicker2.Text;
        }

        private void btnRent_Click(object sender, EventArgs e)
        {
            //Step 1: Add new data into Rental table.

            //Insert statement
            string sqlStr = string.Format("insert into Rental(RentalID,HouseID,TenantID,RentStartDate,RentEndDate,MonthlyRent,DepositPaid,IsReturnDeposit,Remark) values('{0}','{1}','{2}','{3}','{4}','{5}','{6}','{7}','{8}')",
                this.tbRentalID.Text, this.tbHouseID.Text, this.tbTenantID.Text, Convert.ToDateTime(this.tbStartDate.Text), Convert.ToDateTime(this.tbEndDate.Text), this.tbMonthlyRent.Text, this.tbDepositPaid.Text, this.cmbIsReturnDeposit.Text, this.tbRemark.Text);
            //Execute the SQL statement by using the ExecuteCmd method of SQLHelper
            int n = SQLHelper.ExecuteCmd(sqlStr);

            //Step 2: Modify the IsVacant value to "No" in HouseInfo table.
            ...
            if (n > 0 && n1 > 0)
            {
                MessageBox.Show("Rent is success.", "Notification", MessageBoxButtons.OK, MessageBoxIcon.Information);
                DataSet ds1 = new DataSet();
                ds1 = this.QueryAll();
                this.dgvRentalList.DataSource = ds1.Tables[0];
            ...
            {
                MessageBox.Show("Rent is failure.", "Notification", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        ...
        private void btnUpdate_Click_1(object sender, EventArgs e)
        {
            ...
            string sqlStr = string.Format("update Rental set HouseID='{0}',TenantID='{1}',RentStartDate='{2}',RentEndDate='{3}',MonthlyRent='{4}',DepositPaid='{5}',IsReturnDeposit='{6}',Remark='{7}' where RentalID = '{8}'",
               this.tbHouseID.Text, this.tbTenantID.Text, Convert.ToDateTime(this.tbStartDate.Text), Convert.ToDateTime(this.tbEndDate.Text), this.tbMonthlyRent.Text, this.tbDepositPaid.Text, this.cmbIsReturnDeposit.Text, this.tbRemark.Text , this.tbRentalID.Text );
            //Execute the SQL command.
            int n = SQLHelper.ExecuteCmd(sqlStr);
